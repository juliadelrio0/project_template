...
python3 hpo_genes_fetcher.py --hpo "$HPO_CODE" --dir "$RESULTS_DIR/genes_${HPO_SHORT}"

# Ejecutar el script de Python fetch_interactions.py
echo "Ejecutando fetch_interactions.py con el archivo de genes generado..."
python3 fetch_interactions.py --i "$RESULTS_DIR/genes_${HPO_SHORT}/genes_list.txt" --dir_out "$RESULTS_DIR" --dir_img "$RESULTS_DIR/genes_$HPO_SHORT/IMG"

# Ejecutar el script de R plot_network.R
echo "Ejecutando plot_network.R para generar las gráficas..."
Rscript plot_network.R --i "$RESULTS_DIR/interacciones_proteinas.csv" --dir_res "$PLOTS_DIR" --dir_img "$PLOTS_DIR/IMG" --dir_genes "PLOTS_DIR/GENES"
